...
3 to 10, inclusive. C<getopts> is an iterator. The variable C<OPTIND> stores
the position of the next element of the command-line argument list to be
searched for an option name.

The C<getopts( $;$ )> function of the L<Getopts::Std> Perl core module
provides an implementation of the C<getopts> utility. It isn't an iterator
function but processes the command-line argument list in a single call.
C<Getopt::Guided> exposes a new implementation through the C<getopts( $\%;\@
)> function. The primary implementation differences are
...
character, the option should have an option-argument which either shall
be supplied as a separate argument (C<( '-a', 'foo' )>) or shall be part
of the same argument string as the option name (C<( '-afoo' )>).  We call
an option whose option name is followed by a colon/comma "option-argument
indicator" a "common option"/"list option". A list option is sometimes called
"multi-value option".  The POSIX standard knows only common options.
...
function. It is an empty hash (passed by reference) that stores the processing
...
guidelines|https://pubs.opengroup.org/onlinepubs/9799919799/basedefs/V1_chap12.html#tag_12_02>
4 to 10, inclusive. The C<getopts()> function warns and returns false if a
processing error occurs; otherwise the function returns true. The processing
error types are:
...
The 2nd processing error occurs if either no argument or the C<undef> value
is provided for an option that has to have an option-argument.  If any of
the processing errors occurs, the C<getopts()> function restores C<@ARGV>
...
each occurrence.  Use case: A verbosity level can be represented by a C<v+>
incrementable flag.
...
arguments. It replaces the implicit use of C<@ARGV>.

=item *

As of version 1.1.0 this module provides the additional function C<getopts3(
\@$\% )>.  This function has a 3rd "leading" parameter. This parameter is an
array (passed by reference) of command-line arguments. It replaces the implicit
use of C<@ARGV>. This module exports the C<getopts3()> function on request too.
