...
Some options require an argument (option-argument). If an option requires
an argument, an indicator (option-argument indicator) character is attached
to its name. A "common option" has a colon (C<:>) indicator. A "list option"
has a comma (C<,>) indicator. The POSIX standard knows only common options.
Although the POSIX standard allows common options with an optional argument,
this implementation doesn't support that.
