...
by the C<getopts()> function.  The command-line arguments list C<@argv>
is its first (mandatory) parameter.  It must be passed by reference. A
possible value of this parameter is the global C<ARGV> array.  The remaining
...
(an odd-numbered element) that follows such an option specification is
called its "destination". The destination has to be a SCALAR, ARRAY, or
CODE reference.
