...
4 to 10, inclusive. C<getopts()> croaks if a processing error occurs;
otherwise the function returns nothing (void context). The processing errors
are:

=over 2

=item * An option is unknown

=item * An option that requires an option-argument has none

=item * An option with option-argument isn't last one in a group (cluster)
