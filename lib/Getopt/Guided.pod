...
The C<getopts( $;$ )> function of the L<Getopt::Std> Perl core module
...
In Perl the array L<perlvar/@ARGV> contains the command-line arguments.
...
If an option name (C<a>) is followed by a colon (C<:>) character or a comma
(C<,>) character, the option should have an option-argument which either
shall be supplied as a separate argument (C<-a foo>) or shall be part of the
same argument string as the option name (C<-afoo>).  We call an option whose
