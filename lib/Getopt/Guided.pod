...
an argument, an indicator (option-argument indicator) is attached to its
name. This indicator is either a colon (C<:>) or a comma (C<,>). The POSIX
standard knows only the colon indicator.

An option that requires no argument is called a flag.  A flag can
(optional!) have an indicator (flag indicator) too. This indicator is either
an exclamation mark (C<!>) or a plus sign (C<+>). The POSIX standard doesn't
know the flag indicator feature.
...
The first parameter is the "options specification". It is a string that contains
a joined list of options (C<E<lt>nameE<gt>[ E<lt>indicatorE<gt> ]>).

The second parameter is the "name-value map". It is an empty hash (passed
by reference) that stores the C<getopts()> processing result. Each hash
key refers to an option name and the corresponding hash value refers to the
option-argument value (or C<!!1> if it is a flag).
