...
a hyphen-minus (C<->) character.

=head2 OPTIONS FOLLOWED BY AN ARGUMENT

Some options require an argument (option-argument). If an option has a
mandatory argument, an indicator (option-argument indicator) character is
attached to its name. A "common option" has a colon (C<:>) indicator. A
"list option" has a comma (C<,>) indicator. The POSIX standard knows only
common options.  Although the POSIX standard allows common options with an
optional argument, this implementation doesn't support that.
...
An option that requires no argument is called a flag (or switch).  A flag can
optionally have an indicator (flag indicator) character. A "negatable flag"
has an exclamation mark (C<!>) indicator. An "incrementable flag" has a plus
sign (C<+>) indicator. The POSIX standard doesn't know flag indicators.
...
of C<getopts( $\% )> where the second parameter is not a hash reference but
a hash.
...
\@$\% )>.  This function has a 3rd "leading" parameter. The parameter is an
array (passed by reference) of command-line argument values. It replaces the
implicit use of C<@ARGV>. This module exports the C<getopts3()> function on
request too.
